...
class PreClasifyerService:
    ...
    async def classify_message(self, message: str, context: dict | None = None) -> dict:
        ...
        try:
            ...
            logger.exception("[guard] classification failed: %s", exc)
            return {"is_travel_related": False, "is_safe": False}

        is_travel_related = bool(parsed.get("is_travel_related", False))
        is_safe = bool(parsed.get("is_safe", False))
        if not is_safe:
            is_travel_related = False

        return {"is_travel_related": is_travel_related, "is_safe": is_safe}
